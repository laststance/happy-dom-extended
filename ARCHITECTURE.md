...
The public Vitest package is a custom `Environment` named `happy-dom-extended` (`vitest-environment-happy-dom-extended`) for Vitest 4 and 5. In `forks` and `threads`, `setup` constructs `GlobalWindow || Window` with the owned adapter already in `settings`, calls {@link installCompatibility}, then copies Window APIs onto the worker global with Vitest's `populateGlobal` through {@link populateWindowGlobals}, including Node-overlapping keys. Node 25+ defines `localStorage` and `sessionStorage` as lazy getters, and Vitest 4 reads each overridden key's value first, so those getters are replaced with inert placeholders beforehand and never run. {@link importPopulateGlobal} loads populateGlobal from `vitest/runtime` and falls back to `vitest/environments` only when Vitest's own manifest does not export `vitest/runtime` (Vitest 4.0), so Vitest 4.1 prints no deprecation warning. Teardown restores the property descriptors captured before setup. In `vmThreads` and `vmForks`, `setupVM` returns the Window itself as the VM context. Both paths join with {@link ExtendedCanvasAdapter.drain} then `window.happyDOM.close()`, not `abort()`.

The Vitest package also exports `vitest-environment-happy-dom-extended/global-setup`, a separately built entry for Vitest's `globalSetup`. Its {@link setup} calls {@link loadSkiaCanvas} in Vitest's main thread before `threads` or `vmThreads` workers start. Node releases a worker thread's handle to skia.node when the thread exits, and Windows unloads the binary when no handle remains, even while threads that skia-canvas started still run its code. The main thread's handle lasts until the process ends. Importing the entry loads nothing, so the package keeps `sideEffects: false`.

The private `packages/compat` workspace owns runner-independent API repairs and native Canvas integration. tsdown bundles it into each public runtime and a private Worker bootstrap. Happy DOM is pinned to 20.14.0 because the integration uses internal lifecycle hooks. CPU Skia, Culori, WebIDL conversions and bounded image-header inspection remain external runtime dependencies. Canvas modules load skia-canvas through one CommonJS `require` in {@link loadSkiaCanvas}, so a skipped install script produces approval guidance instead of Node's bare `Cannot find module '../skia.node'`. That loader lives in its own module without load-time side effects, so the Vitest global setup can reuse it. Installed-tarball checks verify their presence, shared Happy DOM class identity and the Worker bootstrap path.
