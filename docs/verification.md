...
The full local gate ran on macOS arm64 with Node.js 24.19.0, pnpm 12.3.4 and npm 12.0.2. Source integration used Jest 30.5.1 and Vitest 5.0.1. Installed consumers used Jest 30.0.0 and 30.5.1 plus Vitest 4.0.0, 4.1.11 and 5.0.1, installed by npm 12.0.2 after their generated manifests approved Skia's install script. Test output, not a zero process exit alone, determines completion.

| Layer                                 | Expected successful execution                                                                                                                                                 |
| ------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Node source regressions               | 226 tests, including the Vitest entry loader and the skia-canvas install guidance                                                                                             |
...
| Installed consumer per Vitest version | 7 lifecycle tests; 10 Vitest tests in 3 files with one `forks` worker and with two workers in every pool; 7 React product tests in 4 files in every pool                      |
...
`pnpm test` reported 99.31% source line coverage. `pnpm test:package` took about 100 seconds for all eight installed consumers.
...
| ------- | -------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- |
| 10.33.4 | Exit 0 with an "Ignored build scripts" warning; no binary                  | Interactive `pnpm approve-builds`, which ignores package names; `allowBuilds` or `onlyBuiltDependencies` in `pnpm-workspace.yaml` |
