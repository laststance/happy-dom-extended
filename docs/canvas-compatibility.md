...
Additional owned tests cover CORS on two real origins, video decode/seek/replacement, malformed input, resource ceilings, startup errors, live-child shutdown, native message listeners, receiver Window brands and partial-install restoration. Installed-tarball tests execute both supported Jest and Vitest versions in serial and two-process modes, including the private Worker bootstrap from a path containing spaces. The GitHub test matrix is the authority for Linux/Windows and Node 22/24/26 results; a local macOS pass alone is not evidence that all six CI combinations passed.
