...
Seven OpenSSF Scorecard findings are open, and three of them are deliberate. Scorecard reports the Release workflow's checkout of the Test run's commit as an untrusted code checkout; that job runs only after a successful Test on a push to `main`, and the step straight after checkout stops the run unless that commit is still `origin/main`, before any repository code executes. Scorecard reports the Test workflow's global npm install as unpinned; the npm CLI installs a global package by version and offers no integrity option, while every remote action reference and the package manager itself are pinned by digest. Scorecard reports no dependency update tool; runtime dependencies are pinned to the exact versions the verification evidence covers, so an automatic bump would replace that evidence without re-running it, and [TODOS.md](TODOS.md) tracks that decision. The remaining findings concern the repository's age, its review history, a best-practices badge and branch protection.
