...
test('P3 ImageData keeps Vitest worker arrays and real colors through readback and pixel replacement', () => {
  // Arrange
...
test('Canvas draws ImageData from Vitest worker subarrays and returns the same Window image family', () => {
  // Arrange
...
test('Consumers can spy on real Canvas methods and restore drawing without replacing the context', () => {
  ...
  canvas.height = 1
  const drawing = canvas.getContext('2d')!
  const spy = vi.spyOn(drawing, 'fillRect')
  // Act
...
test('Real Canvas output completes even while Vitest fake timers are enabled', async () => {
  // Arrange
  const canvas = document.createElement('canvas')
  canvas.width = 1
  canvas.height = 1
...
test('Restoring Canvas method descriptors preserves Window ImageData and source Canvas drawing', () => {
  ...
  const replacement = vi.fn<typeof drawing.createImageData>(
    () => new ImageData(1, 1),
  )
  Object.defineProperty(drawing, 'createImageData', { value: replacement })
  Object.defineProperty(drawing, 'drawImage', { value: vi.fn() })
  Object.defineProperty(drawing, 'putImageData', { value: vi.fn() })
...
test('bitmap creation, decoded Blob URLs and binding failures retain the Vitest Window realm', async () => {
  // Arrange
...
test('structured Canvas transfers keep Vitest Window brands and detach the original canvas and bitmap', () => {
  // Arrange
