...
for (const storage of ['ArrayBuffer', 'SharedArrayBuffer'] as const) {
  test(
    `generated Vitest worker ImageData retains ${storage} subviews and draws mutations at their original offset`,
    () => {
      fc.assert(
        fc.property(
          fc
          ...
          ({
            ...
            // Arrange: these constructors execute inside the actual Vitest worker.
